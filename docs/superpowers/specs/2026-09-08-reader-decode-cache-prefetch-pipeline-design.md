...
| §11 big-strip band decode | **implemented 2026-09-12**: `docs/superpowers/specs/2026-09-09-reader-webtoon-strip-band-decode-design.md` (rev 5) + its `-plan.md`, 9 steps, branch `claude/reader-backlog-batch-c` | `StripDecodeSession` (SkiaSharp scanline decode - `GetPixels(..., Subset)`, the design's own original mechanism, turned out unsupported by Skia's own JPEG/PNG codecs; discovered by running it, not just reading source) + `_bandCache`/`_stripSessions` on `ReaderImagePipeline`, `SetStripBandWindow`/`TryGetCachedBand`/`IsKnownBandableStrip`/`RequestPageSize`/`PageSizeAvailable` on `IReaderPageSource`, band-slot rendering in `ReaderPageVisualHandler`. **JPEG-only** - PNG strips still get the header-only size-peek benefit but fall back to whole-page decode (Skia genuinely has no working row-range decode for PNG in this build). Found and fixed a real, reproducible native crash during implementation: a session's `SKCodec` disposal raced its own in-flight scanline decode call across two different locks. |
