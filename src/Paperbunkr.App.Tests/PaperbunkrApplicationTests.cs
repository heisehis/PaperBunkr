...
public sealed class PaperbunkrApplicationTests : IDisposable
{
    ...
    [Fact]
    public void GetComicFormatIcon_UnrecognisedFormat_ReturnsNull()
    {
        // An unrecognised format string never resolves to a table row at all, so ResolveFormat
        // can't reach its SvgAsset branch - exercises the same null-safety path the other icon
        // methods use. (A recognised format like "Trade Paperback" now has a real bundled asset,
        // Assets/Marks/Formats/trade-paperback.svg, since the brand-iconography work landed - this
        // test originally used that value, back when no format had a bundled asset yet.)
        int seriesId = AddSeries("Unknown Format Series");
        int issueId = AddIssue(seriesId, "1", format: "Not A Real Format Xyz");
        var app = new PaperbunkrApplication(new MainViewModel());
        var issue = app.GetBook(issueId)!;
