# Keyboard Shortcuts

Every reader shortcut below is **remappable** in **Preferences → Reader → Keyboard
Shortcuts**. Defaults mirror ComicRack CE's keymap (with two deliberate exceptions —
page-turn is Left/Right instead of CE's PageUp/Alt+Left).

## Navigation

| Action | Default | Where it applies |
|---|---|---|
| Page back (spatial left) | `←` | paged, not zoomed |
| Page forward (spatial right) | `→` | paged, not zoomed |
| Pan left / right / up / down | `←` `→` `↑` `↓` | paged, zoomed in |
| Scroll left / right / up / down | `←` `→` `↑` `↓` | continuous modes |
| Scroll up / down a page | `PageUp` / `PageDown` | continuous modes |
| Scroll to start / end | `Home` / `End` | continuous modes |
| Toggle auto-scroll | `S` | continuous modes |
| Previous bookmark | `Ctrl+PageUp` | always |
| Next bookmark | `Ctrl+PageDown` | always |

## Zoom & Fit

| Action | Default |
|---|---|
| Zoom in | `Z` |
| Zoom out | `Shift+Z` |
| Fit: Original size | `1` |
| Fit: Fit all | `2` |
| Fit: Fit width | `3` |
| Fit: Fit height | `4` |
| Fit: Best fit | `5` |

## Display

| Action | Default |
|---|---|
| Toggle fullscreen | `F` |
| Rotate clockwise | `R` |
| Rotate counter-clockwise | `Shift+R` |

## Everywhere

- **Tab / Shift+Tab** — move focus between controls
- **Enter / Space** — activate the focused control
- **Esc** — cancel or close the current editor / overlay

## Remapping

1. **Preferences → Reader → Keyboard Shortcuts**.
2. Click a command's key field and press the new combination.
3. Conflicts are flagged inline (a shortcut that would shadow another in the same reader
   state).
4. **Import** / **Export** buttons save and load your whole keymap as a file — handy for
   moving between machines or sharing a layout.
