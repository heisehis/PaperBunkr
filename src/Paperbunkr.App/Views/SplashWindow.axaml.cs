...
///
/// The XAML keyframe animations animate <c>ScaleTransform.ScaleX/Y</c> against an explicit
/// <c>&lt;ScaleTransform&gt;</c> (the Avalonia-documented way) - animating <c>RenderTransform</c>
/// directly has no registered animator in this build and crashes at construction. Reduced motion
/// (<see cref="SkinService.GetReducedMotion"/>) removes the <c>.animate</c> class, which deactivates
/// those animation styles; the check is here rather than via the <c>PbMotion*</c> resources, which
/// have not necessarily been zeroed yet at splash time.
/// </summary>
public partial class SplashWindow : Window
{
    ...
    public SplashWindow(bool reducedMotion)
    {
        ...
        if (reducedMotion)
        {
            LogoImage.Classes.Remove("animate");
            GlowBorder.Classes.Remove("animate");
            LogoImage.Opacity = 1;
            GlowBorder.Opacity = 0.55;
        }
    }
