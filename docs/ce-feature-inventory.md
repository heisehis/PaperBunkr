...
| Page layout: single / double-page spread / adaptive spread detection | ✅ shipped — `PageLayoutMode`/`ToggleDoublePageMode`/`SpreadLayoutMath` (docs/superpowers/specs/2026-08-15-reader-double-page-spread-design.md); CE's 3 modes deliberately collapsed to 2 (`Double` behaves like CE's `DoubleAdaptive`). Per-page manual Near/Far override **shipped 2026-09-11** — `PageSpreadPosition` on `IssuePage`, set via a reader context-menu submenu (no `PagesView`-style dedicated screen; CE parity confirmed: the flag only nudges `NextPage`/`PreviousPage` step size, it never changes the pairing test itself) plus read-only import from embedded `ComicInfo.xml`'s `<Page Position="…">` on fresh scan. docs/superpowers/specs/2026-09-10-reader-backlog-batch-b-design.md. |
...
| Background: solid/texture/paper-texture, page margins | ⚠️ partial — solid-color background + page margins shipped (`AppSettings.ImageBackgroundMode`/`PageMarginEnabled` etc., global-only by design, no per-issue override). Tiled background texture mode **shipped 2026-09-11** — `ImageBackgroundMode.Texture` + 3 bundled seamless swatches (neutral-dark/carbon/linen), `ImmutableImageBrush`-style tiling via `ImageBrush.ToImmutable()`, plus a texture-only page drop-shadow for depth (deliberate deviation, not CE-sourced — see the design doc's visual-companion decisions). docs/superpowers/specs/2026-09-10-reader-backlog-batch-b-design.md. CE's other mode — an actual **paper texture rendered over the page image itself** (not just the canvas background) — remains a genuine, separate gap; not attempted here. |
...
| Split-page "part" navigation for zoomed pages, type-to-jump-to-page, scroll page-turn throttling | ⚠️ partial — page-turn throttling shipped (`ReaderScreenViewModel`'s rapid-paging guard); split-page part navigation **shipped 2026-09-05** (`PagePartMath.cs` — this row previously claimed CE has "no Part concept, confirmed absent," which was wrong: verified directly against `ImageDisplayControl.cs`/`ComicDisplay.cs` source, CE genuinely splits an overflowing page into a grid of viewport-sized parts and steps through them before turning the page; ported, including the double-page-spread case, docs/superpowers/specs/2026-09-05-reader-polish-backlog-finish-design.md §1); **type-to-jump-to-page shipped 2026-09-10 as a deliberate deviation, not parity** — verified against `_reference/ComicRackCE` that CE has *no* numeric go-to-page anywhere (no dialog, field, or digit-key seek; CE's `D1`–`D0` are all zoom/layout modes), so this row's "genuinely not built" framing was misleading. Paperbunkr adds it in the spirit of "more than CE": `ReaderGoToPage` command (default `G`) + an inline `TextBox` that replaces the `PAGE x / y` readout (docs/superpowers/specs/2026-09-10-reader-backlog-batch-a-design.md Item 3) |
...
- **Reader (§B):** magnifier/loupe, sharpen/AutoContrast/WhitePoint image adjustment,
  type-to-jump-to-page (designed on unmerged branch `claude/reader-backlog-batch-a`, not landed).
  (Clock/battery overlay and split-page part navigation shipped 2026-09-05; tiled background
  texture mode and a per-page manual Near/Far double-page override shipped 2026-09-11 — see the
  two rows above. CE's *paper texture drawn over the page image itself*, distinct from the
  canvas-background texture we built, is still a genuine gap.)
