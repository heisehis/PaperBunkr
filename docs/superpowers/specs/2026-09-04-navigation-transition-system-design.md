...
| [ReaderScreen](../../../src/Paperbunkr.App/Views/ReaderScreen.axaml) | first-page surface | `ImageSource` = first page `Bitmap` straight from `PageImageDecoder` (already in memory). Destination rect = the Reader's computed page-fit rect if the layout model exposes it cheaply, else `PageCanvas` bounds. **Kept in v1 on the understanding that if the `PageCanvas` (`CompositionCustomVisualHandler`) integration looks janky on screen, this one participant is cut and Reader keeps only the push/pop fade+scale.** |
