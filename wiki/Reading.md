...
- **Fit modes** (toolbar, or keys `1`–`5`): *Original*, *Fit All*, *Fit Width*,
  *Fit Height*, *Best Fit*.
- **Zoom**: `Z` in / `Shift+Z` out, mouse wheel (configurable), or **zoom presets**
  (100 / 125 / 150 / 200 / 400 %).
- When zoomed in, the arrow keys **pan** instead of turning the page.
...
Mark a page as **Story / Cover / Advertisement / Deleted** (Reader Tools → *Page Type*).
A badge shows the current page's type. Deleted pages are skipped in reading.

## Image adjustment

**Reader Tools → ADJUST** gives live sliders for **Brightness, Contrast, Saturation,
Gamma**, applied to the rendered page in real time. Background colour and page margins are
in [Preferences → Reader](Preferences).
