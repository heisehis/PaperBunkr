...
/// <summary>
/// Verifies the <c>AddReaderAutoHideChrome</c> migration - a single plain <c>AppSettings</c> bool
...
/// <see cref="Paperbunkr.Data.Entities.AppSettings.ReaderAutoHideChrome"/>'s own doc comment) -
...
/// </summary>
public class AddReaderAutoHideChromeMigrationTests : IDisposable
{
    private const string PriorMigration = "20260914220638_ConsolidateLibraryViewModes";
    private readonly string _dbPath;

    public AddReaderAutoHideChromeMigrationTests()
    {
        _dbPath = Path.Combine(Path.GetTempPath(), $"paperbunkr_readerautohide_migration_test_{Guid.NewGuid():N}.db");
    }
    ...
    public void Migration_ColumnDefault_IsTrue_NotFalse()
    {
        ...
        string? dfltValue = context.Database
            .SqlQueryRaw<string>("SELECT dflt_value AS Value FROM pragma_table_info('AppSettings') WHERE name = 'ReaderAutoHideChrome'")
            .Single();
    ...
    public void Migration_FreshRow_DefaultsToTrue_AndRoundTrips_AndIsReversible()
    {
        using (var context = CreateContext())
        {
            ...
            Assert.True(settings.ReaderAutoHideChrome);

            settings.ReaderAutoHideChrome = false;
            context.SaveChanges();
        }
        ...
        {
            var settings = context.GetOrCreateAppSettings();
            Assert.False(settings.ReaderAutoHideChrome);
        }
