...
| [ReaderScreen](../../../src/Paperbunkr.App/Views/ReaderScreen.axaml) | `PageCanvas` (the whole control, not a computed fit-rect) | **Landed 2026-09-05**, reversing an earlier "cut" call in this same pass - `PageCanvas` turned out to already expose a public `Page` (`Bitmap?`) `StyledProperty`, already bound (`Page="{Binding CurrentPage}"`) - no new plumbing needed for the image. `ReaderScreenViewModel.SharedElementKey` (`"issue-cover:{issueId}"`, mirrors `IDetailHeaderSource.SharedElementKey`) is the key. Destination rect is `PageCanvas.Bounds` (the whole control), not a computed zoom/fit-mode-aware sub-rect - the pre-approved fallback, since precise fit geometry isn't cheaply available from a custom-drawn `Control`. |
