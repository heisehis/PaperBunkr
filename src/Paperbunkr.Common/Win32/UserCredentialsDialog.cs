...
{
	// TODO(Paperbunkr): originally derived from System.Windows.Forms.CommonDialog (for
	// ShowDialog()/RunDialog(hwndOwner) integration with the WinForms message loop). Rewritten as
	// a plain class exposing ShowDialog(IntPtr) directly -- the underlying behavior is a native
	// CredUIPromptForCredentials call keyed off a raw HWND, nothing WinForms-specific about it.
	public class UserCredentialsDialog : IDisposable
	{
		...
		/// <summary>Shows the native credential prompt. Returns true if the user confirmed (OK), false on cancel.</summary>
		public bool ShowDialog(IntPtr hwndOwner = default)
		{
		...
		public void Reset()
		{
			target = cYo.Common.Runtime.ApplicationInfo.ProductName ?? AppDomain.CurrentDomain.FriendlyName;
			user = null;
			password = null;
			domain = null;
			caption = null;
