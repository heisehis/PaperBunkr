...
namespace cYo.Projects.ComicRack.Engine.IO.Provider
{
    ...
    {
        // Written fresh for Paperbunkr (docs/onboarding.md §2/§4): the DllImport target below
        // was already the portable bare-name pattern in CE (no baked-in path/extension), so this
        // mainly adds the explicit resolver registration used consistently across all five native
        // shims instead of relying only on default OS search-path behavior.
        public static class NativeMethods
        {
            static NativeMethods()
            {
                Native.NativeInterop.RegisterResolver(typeof(NativeMethods).Assembly, "libheif");
            }
